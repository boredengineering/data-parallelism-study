...
def worker(local_rank, args):
    ...
    # processes is equivalent to the number of available GPUs per node) with mp.spawn().  
    # Input parameters for mp.spawn() include the worker function, the number of GPUs per node (nprocs), 
    # and all the parsed arguments.
